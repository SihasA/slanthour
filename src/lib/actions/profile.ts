...
export async function updateProfile(input: ProfileInput): Promise<ActionResult> {
  ...
  const { data: current } = await supabase
    .from("profiles")
    .select("username, display_name, website_url, instagram_handle, email_public")
    .eq("id", user.id)
    .single();
  ...
  } else if (displayNameChanged || contactChanged) {
    // The display name and the opt-in contact links are rendered on every
    // published page footer from the cached profile join, so a change to
    // either must invalidate each page's tag or the old value lingers up
    // to an hour.
    const { data: pages } = await supabase.from("pages").select("slug").eq("user_id", user.id);
    for (const row of pages ?? []) {
